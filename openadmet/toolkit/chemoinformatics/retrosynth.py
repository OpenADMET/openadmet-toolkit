...
from openadmet.toolkit.chemoinformatics.rdkit_funcs import run_reaction, smiles_to_inchikey
...
class ReactionSMART(BaseModel):
    ...
    reaction_name: str = Field(
        ..., title="Name", description="The name of the reaction"
    )
    product_names: list[str] = Field(
        ..., title="Fragment names", description="The names of the products"
    )
    reactant_names: list[str] = Field(
        ..., title="Reactant names", description="The names of the reactants"
    )
...
class BuildingBlockCatalouge(BaseModel):
    building_block_csv: Union[str, Path] = Field(
        ...,
        title="CSV files",
        description="The CSV file containing the building block information",
    )
    smiles_column: str = Field(
        "SMILES",
        title="SMILES column",
        description="The name of the column containing the SMILES strings",
    )
    inchikey_column: str = Field(
        "INCHIKEY",
        title="INCHIKEY column",
        description="The name of the column containing the INCHIKEY strings",
    )
    vendor_column: str = Field(
        "VENDOR",
        title="Vendor column",
        description="The name of the column containing the vendor information",
    )
    ...
    subselect_vendors: list[str] = Field(
        None,
        title="Vendor subselection",
        description="The list of vendors to subselect",
    )
    ...
    def load(self) -> pd.DataFrame:
        ...
        df["combined_id"] = (
            df[self.vendor_column].astype(str)
            + "_"
            + df[self.compound_id_column].astype(str)
        )
...
class Retrosynth(BaseModel):
    reaction: ReactionSMART = Field(
        ..., title="Reaction", description="The reaction to run"
    )
    ...
    def run(self, df: pd.DataFrame, smiles_column: str = "SMILES") -> list[str]:
        ...
        for i, product_name in enumerate(self.reaction.product_names):
            df[f"{self.reaction.reaction_name}_{product_name}"] = df[
                f"{self.reaction.reaction_name}_products"
            ].progress_apply(lambda x: x[i] if x is not pd.NA else pd.NA)
            # add inchikeys for the products also
            df[f"{self.reaction.reaction_name}_{product_name}_inchikey"] = df[
                f"{self.reaction.reaction_name}_{product_name}"
            ].progress_apply(
                lambda x: smiles_to_inchikey(x) if x is not pd.NA else pd.NA
            )
...
class BuildingBlockLibrarySearch(BaseModel):
    ...
    def run(
        ...
        synth_data = df.progress_apply(
            lambda row: self.row_search_library(
                row,
                building_block_df,
                self.building_blocks.inchikey_column,
                self.reaction,
            ),
            axis=1,
        )
