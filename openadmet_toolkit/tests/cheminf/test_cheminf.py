import pytest
import pandas as pd
from openadmet_toolkit.cheminf.rdkit_funcs import run_reaction, smiles_to_inchikey, standardize_smiles
from openadmet_toolkit.cheminf.retrosynth import ReactionSMART, BuildingBlockCatalouge, BuildingBlockLibrarySearch, Retrosynth
...
@pytest.fixture()
def amide_dataframe():
    data = pd.DataFrame({"SMILES": ["CCN(CC)CC(=O)NC1=C(C=CC=C1C)C"], "INCHIKEY": ["NNJVILVZKWQKPM-UHFFFAOYSA-N"], "common_name": ["lidocaine"]})
    return data 
...
def test_reaction_smart_retro(amide_dataframe):
    rs = ReactionSMART(reaction="[C:1](=[O:2])[N:3]>>[C:1](=[O:2])[OX2H1].[N:3]", reaction_name="amide_coupling", product_names=["carboxylic_acid", "amine"], reactant_names=["amide"])
